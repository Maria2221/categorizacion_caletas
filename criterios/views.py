...
def resultadoA(request):
	...
	contexto = {'form':ResultForm,'tipo':"Categoria I", 'nivel':"Productividad Alta",'ponderado':value,'condicion':condicion}
	return render(request, 'resultado.html',contexto)

def resultadoB(request):
	value = request.GET.get('value', '')
	condicion = request.GET.get('condicion','')
	contexto = {'form':ResultForm,'tipo':"Categoria II", 'nivel':"Productividad Mediana",'ponderado':value,'condicion':condicion}
	return render(request, 'resultado.html',contexto)

def resultadoC(request):
	value = request.GET.get('value', '')
	condicion = request.GET.get('condicion','')
	contexto = {'form':ResultForm,'tipo':"Categoria III", 'nivel':"Productividad Media Baja", 'ponderado':value,'condicion':condicion}
	return render(request, 'resultado.html', contexto)

def resultadoD(request):
	value = request.GET.get('value', '')
	condicion = request.GET.get('condicion','')
	contexto = {'form':ResultForm,'tipo':"Categoria IV", 'nivel':"Productividad muy Baja",'ponderado':value,'condicion':condicion}
	return render(request, 'resultado.html',contexto)
...
def saveResult(request):
	...
	if form.is_valid():
		...
		register = usuariomodelo.formulario_set.create(nombreFormulario=form.cleaned_data['nombreFormulario'],tipo= request.POST.get('tipo') , 
					nivel= request.POST.get('nivel'), ponderado = ponderado, condiciones= condiciones) # condiciones = arreglo de los ponderados de condiciones
		#print(ponderado)
		register.save()
